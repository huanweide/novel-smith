...
9. **插件 API 草案**：让社区接记忆策略 / 导出器 / UI 主题，文档化「加一个插件」最小路径。**【已交付 v3.1.111 草案】** 详见 `docs/plugin-api-draft.md`——基于 v3.1.111 真实代码盘点三个扩展点（导出器 `SUPPORTED_FORMATS` 白名单 / 记忆 `DECAY_RULES` 常量 / 主题 `THEMES` 联合类型），给出统一插件注册表目标形态 + 三类 `register*` 最小路径代码 + 分阶段落地路线（Phase 0 抽 `exporterRegistry` ROI 最高）。纯文档，不 bump。

### P3 · 增长与生态（可选、不阻塞）
10. **游戏模式**：**降级为卫星功能**（董事会 2.0 共识），入口可隐藏，不进主闭环，不自动注入生成上下文。有需求再独立迭代。
11. **移动端响应式阅读模式**：**【已交付 v3.1.107 + v3.1.108 增强】** 新建 `/read/[projectId]` 移动优先阅读路由——目录抽屉 + 沉浸正文 + 字号调节 + 阅读进度 + 翻页，复用既有 chapters/nodes 接口与 MarkdownViewer，零数据层改动；首页项目卡加「阅读」入口。**v3.1.108 增强**：记住每本书上次读到哪一章、夜间/纸感/跟随主题三档护眼主题、键盘 ← → 翻页。
