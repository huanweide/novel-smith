...
### 吸星改造 1：首页零门槛引导 + README 首屏重构（STAR-READINESS-1）

- **新功能**：首页 Hero 区新增零门槛试用引导条。启动时探测 `/api/health` 的 `llm.ok`，**仅当 AI 未配置时显示**——引导访客先去「去 AI 味检测」拿到一次真实体验，而不是在「要配 Key」这一步直接流失。文案强调「完全不需要 Key」「纯本地规则引擎：不联网、不上传、零成本」。
- **诊断依据**：去 AI 味检测是**纯前端实现**（`use client` + `core/humanize`，零 API 调用、零数据库写入），因此**在线 demo 上完全可用**；但此前首页没有任何引导把它暴露给访客，访客点进 demo 只能「看界面」，构成最大的转化断点。
- **文档**：README 首屏（中英同步）重构——删掉劝退式的「在线预览只能看界面、做不了实际操作」，改为价值前置的「**先花 30 秒试一下（不用配 Key、不用安装）**」；新增「哪些功能不需要 Key」对照表；导航行加 30 秒试用直达入口。
- **门禁**：tsc 0 错 · vitest 177 文件 1921 测试全绿 · next build 通过。
## v3.1.134 — 2026-09-12

### 全站 UI 走查收口：右侧面板 tab 断字修复（TAB-WRAP）

- **走查范围**：12 个页面路由 + 写作台 36 个可交互点 + 其余 8 个页面的交互点全量点击验证。
- **修复**：右侧面板顶部 6 个 tab 在 320px 面板内被挤到每个 48px 宽，3 字标签（AI助手 / 工具箱）被迫换行成「AI助/手」「工具/箱」，中文词被拆断。改为可换行布局（tab 容器 `flex-wrap`、每个 tab `flex-1 min-w-[5.5rem] whitespace-nowrap`），6 个 tab 自动排成两行、标签完整显示；最小化按钮独立固定在右上角。
- **走查结论**：全站 12 路由全部 HTTP 200、零 JS 错误；写作台可交互点逐一点击均有响应（分卷/平铺视图、微调模式、专注/沉浸模式、各对话框弹出均正常）；设置 / 检测 / 工坊 / 探讨等页面交互正常；**未发现冗余、重复或失效的交互控件**；全站无横向溢出，章节标题省略属设计内 `truncate`。
- **门禁**：tsc 0 错 · vitest 177 文件 1921 测试全绿 · next build 通过。
## v3.1.133 — 2026-09-12

### 对比模式：任意生成发现原有内容一律左右并排二选一（COMPARE-MODE）

- **新功能**：正文生成 / 精修 / 续写 / 游戏模式导出，只要目标章节**已有内容**（非空），生成完成后进入**左右并排对比**界面，由作者选择保留哪一边，不再静默覆盖原稿。
- **统一判据**：抽成纯函数 `src/lib/compare-mode.ts` —— `shouldEnterCompare` = 原有内容非空 且 新内容非空；原有为空（首次生成）直接采用、不做对比。写作与游戏两条入口共用同一套逻辑与同一个组件。
- **新组件**：`src/components/workspace/CompareModeModal.tsx` —— 左右两栏同时观看（各带字数 / 段落数），点击任一栏即选中高亮，底部按钮落定要保留的一边；未保留的一边仍可在版本历史找回。
- **接入范围**：① 写作主流程的生成完成事件统一改判（原先只有「精修 + 原有内容非空」才弹 diff 预览）。② 游戏模式导出：`endGameAndExport` 回传作者入游时刻的原正文快照，`/api/game/end` 一并透出，导出后原有内容非空即进同一对比界面。
- **安全**：「保留原有」的回写走与手动编辑完全相同的 PUT 通道（带 `undo` 标记，与精修撤销同一条路），受乐观锁保护，不绕过冲突检测。
- **门禁**：tsc 0 错 · vitest 177 文件 1921 测试全绿 · next build 通过。
## v3.1.132 — 2026-09-12

### 十档主题系统：夜航/苍青拉开差距 + 新增 7 套参考配色（THEME-10）

- **新功能**：主题从 3 档扩展到 10 档——深色 7 档（夜航·墨蓝紫 / 苍青·青绿 / GitHub 暗 / 德古拉 / 北境 / 东京夜 / 古旧）+ 浅色 3 档（白昼 / 曜石 Solarized / 拿铁 Catppuccin），各主题底色与主色相互区分。
- **苍青重做**：底色 `#04181A`（青黑）+ 主色 `#5FE3C8`（青绿），与夜航 `#0B1322`（墨蓝紫）明显拉开，解决此前两档过于相似的问题。
- **新主题参考官方色值**：GitHub Dark `#0D1117` / Dracula `#282A36` / Nord `#2E3440` / Tokyo Night `#1A1B26` / Gruvbox `#282828` / Solarized Light `#FDF6E3` / Catppuccin Latte `#EFF1F5`。
- **交互**：切换器菜单按深色/浅色分组、每项带底色色点预览、超高可滚动；根布局防闪烁脚本同步支持 10 档；右键仍可快速循环切换。
- **质量**：所有新主题的四级文字色由生成器按 WCAG 2.1 AA 程序化精算——在三类表面（页面底/容器面/浮起面合成）上均 ≥4.5:1，且层级严格不倒挂；对比度回归守卫从 43 断言扩到 92 断言（主题块 6 → 13 套）。
- **门禁**：tsc 0 错 · vitest 175 文件 1903 测试全绿 · next build 通过。
## v3.1.131 — 2026-09-12
...
- **修复**：写作主面板 CenterPanel 的 3 个 placeholder-only 输入（本节点大纲草稿 / 章纲预览提示词 / 作者指令·微调指令）原先只有占位提示、读屏念不出字段名、无语义关联；现补 aria-label 让读屏能正确念出字段名。
- **决策**：这 3 处是「只有 placeholder」型控件，没有兄弟可见文字标签，补可见 FormLabel 会挤占紧凑的写作工具栏版式；按 v3.1.126 一致性录入行的既定决策，用 aria-label 兜底，不破坏版式。
- **门禁**：tsc 0 错 · vitest 172 文件 1839 测试全绿 · next build 通过。CenterPanel 依赖 useWriterStore + 30+ props，完整渲染测试需构造复杂 selectedNode 易假绿，依 v3.1.126 先例未写脆弱单测，改用 tsc + 既有 1839 测试 + 黑箱核心旅程 + grep 验证兜底。

## v3.1.128 — 2026-09-12

### 表单标签横扫（四轮）：导出/审稿面板补齐字段标签关联（FORM-LABEL-SWEEP-4）

顺着 v3.1.125 / v3.1.126 / v3.1.127 同类的「输入框没有可被读出的字段名」问题，这一轮把导出 / 审稿面板（PublishCheckPanel）过了一遍。

- **6 个会提交的控件（目标平台 / 导出格式 / 审稿角色 / 审稿范围 / 选择章节 / 提示词（可编辑））**：原先是兄弟 `<span>` 文字 + 无 `id` 的控件——**看得见、读屏念不出、点文字不聚焦**。现把 `<span>` 改为 `<label htmlFor>`、给每个控件补 `id`，一一对应关联。
- **「选择章节」** 仅在审稿范围选「指定单章」时出现，且触发拉取章节列表——属条件渲染，本轮回退测试不覆盖，但 `htmlFor` / `id` 关联已就位。
...
### 表单标签横扫（三轮）：项目设定对话框补齐字段标签关联（FORM-LABEL-SWEEP-3）

顺着 v3.1.125 / v3.1.126 同类的「输入框没有可被读出的字段名」问题，这一轮把「项目设定」保存对话框（BuildConfigDialog）过了一遍。

- **9 个配置字段（书名 / 类型 / 受众 / 字数 / 情节结构 / 风格偏好 / 力量体系 / 金手指 / 核心冲突）**：原先是裸 `<label>` 兄弟节点 + 无 `id` 的控件——**看得见、读屏念不出、点标签不聚焦**。现给每个控件补 `id`、给 `Field` 补 `htmlFor`，一一对应关联。
- **流派标签搜索框**：原先只有 `placeholder`、无可见标签也无 `aria-label`，补 `aria-label="搜索流派标签"`，纳入读屏与自动化可定位范围。

### 回归测试

- 新增 `BuildConfigDialog.test.tsx`（4 例）：9 字段均可被 `getByLabelText` 取到且 `id` 与 `htmlFor` 对应；搜索框通过 `aria-label` 获得可访问名；标签的 `htmlFor` 指向真实存在的控件 `id`（关联真实生效）。
...
- **问题（真实不一致）**：世界观模块的新建表单里，每个输入框只有一个灰色 placeholder（「大陆/国家/城市/宗门/秘境/禁地」之类提示语）。**一开始打字提示就消失**，用户填到一半回头看，根本认不出这一格是「类型」还是「所属上层地域」；读屏软件也只能念出提示语、念不出字段名。而同表单的「记忆注入方式」下拉框**早就有自己的标签**——同一个表单里两种待遇。
- **修法**：字段的中文名（类型 / 所属上层地域 / 描述 …）**本来就在数据里**（`MODULE_FIELDS` 的 `f.label`），只是从来没渲染给用户看过——它只被拿去拼正文的【标签】。本次新增 `FormLabel` 组件，用 `<label htmlFor>` + `id` 显式关联，把已有字段名渲染出来，覆盖标题与全部字段；「记忆注入方式」下拉也一并接入同一组件，两类控件统一。

### 提交按钮文案统一：保存 → 创建（BTN-TEXT-INCONSISTENT）

- **问题**：世界书新建表单的提交按钮写的是「保存」，而角色弹窗、章节弹窗都是「创建」。同一个「新建条目」的动作两套词，会让人以为「保存」是保存草稿、而不是新建。
- **修法**：统一为「创建」（图标由 `save` 换成 `plus`，与其它新建入口一致）。

### 回归测试

- 新增 `src/components/workspace/WorldEditor.test.tsx`（6 例）：逐字段断言 `getByLabelText` 能取到标签、标题必填带星号、「记忆注入方式」有标签、提交按钮为「创建」且不存在「保存」、保存中显示「创建中...」并禁用、`showCreate=false` 不渲染标签。

### 前端黑箱新增「核心用户旅程」环节

- `blackbox-ui.cjs` 新增 `runCoreJourney`：真实点击「建角色 → 建世界书 → 手动加章节 → 补正文 → 阅读模式核验 → 回读 `GET /api/projects/[id]` 核对 characters/lorebookEntries 真落库」，全程零 LLM 调用（不碰墨灵 / AI 助手 / 续写等外部模型按钮）；前端断言由 74 项扩至 **91 项**，实测 91/91 全过。
...
- **问题（真实隐患）**：`projects/[id]/lore-tables/[tableId]` 的 PUT 此前 `data: { ...body } as any`、`rules/[id]` 的 PUT 此前 `data: body`，把整个请求体（含 `projectId` / `id` / `createdAt` 等系统字段）直接写库。客户端可借请求体把表格或规则挪到别的项目、篡改主键、倒签创建时间——是典型的越权裸写数据库隐患。
- **修法**：两条路由改为显式字段白名单写入，并去掉 `as any` 恢复类型检查，与 `characters/[id]`、rules POST（`readValidatedBody`）既有约定保持一致；POST 路由本就只取白名单字段且 `projectId` 来自 URL，不受影响。
- **回归测试**：新增 `route.test.ts` 两个，用 mock prisma 断言 PUT 写入的 `data` 只含白名单字段、且经请求体注入的 `projectId` / `id` / `createdAt` 被彻底剥离。
...
- **问题（真实痛点）**：全站 137 个 API 路由里有 71 个直接 `await request.json()`。客户端发畸形 JSON（比如少了个引号、多了个逗号）时，它抛 SyntaxError，一路落到错误分类器的默认分支，变成「服务器内部错误，请查看日志」+ **500**。用户看到 500 会以为服务挂了、还得去翻服务端日志，而真实原因只是自己请求格式写错了——**把客户端的锅甩给了服务器**。
- **修法**：在统一错误分类层 `src/lib/api-error.ts` 的 `classifyError` 里新增 3.6 分支——只要错误是 `SyntaxError` 或消息含 JSON 解析关键词（`Unexpected token` / `is not valid JSON` / `Failed to parse body` 等），就归为 `400 BAD_REQUEST`，并回一条教用户怎么改的提示（JSON 键名与字符串都要用双引号、末尾不能有多余逗号），而不是让他去看日志。
- **为什么是一处改动而不是改 71 个路由**：批量改 71 个文件要逐个适配它们杂七杂八的写法（28 处裸赋值、15 处 `as any`、4 处 `.catch(() => ({}))`、3 处解构），风险高且对未来新路由不生效；统一错误层一处兜底，覆盖全部 137 个路由、对未来也自动生效。路由层用 `safeJson`（语义明确、还能顺带校验字段）仍然推荐，这里只是兜底。
- **不误伤**：配置类中文错误（「API Key 未配置」等）仍走 CONFIG 分支、不被抢走；服务端内部 JSON 解析失败等普通异常仍走 500，且不泄露请求体片段。

### 门禁

- 新增 `src/lib/api-error.test.ts` 7 条测试钉死该行为（含「不泄露请求体片段」「不误伤配置类错误 / 普通异常」边界）。
- 三道门禁：`tsc` 0 错 · `vitest` 157 文件 1771 测试全绿（新增 7）· `next build` 通过。

## v3.1.116 — 2026-09-11

### 给最烧钱的模块补上 70 条测试

**为什么是它**：`src/core/llm/client.ts` 是 core 目录下**唯一长期零测试**的核心模块，727 行，所有 AI 请求都从这里发出去。它默默决定三件事——而这三件事过去没有任何测试看守：

1. **烧多少钱**：失败时重试几次、4xx 要不要重试、要不要切备用模型；
2. **会不会被静默截断**：输出预算（max_tokens）算得够不够；
3. **成本看板准不准**：用量记的账对不对。

- **输出预算**：推理模型强制保底 2500 token——因为思考链和正文共用一份预算，给少了思考链会把预算吃光、**正文直接空掉**（这是踩过的坑）。目标字数按 1.6 倍 token 放大，取 4096 下限、上下文窗口 80% 上限，长章不再被固定预算截断。
- **重试资格**：429 限流 / 5xx / 网络不可达才重试；400 / 401 / 403 / 404 **一次都不重试**——鉴权失败重试只是白等、白记账，切备用模型也救不了填错的 Key。
- **用量记账**：成功按业务角色记；失败的尝试也要记并带 `fail:` 前缀，否则成功率会被算得虚高；走备用模型的调用标记 `isFallback`，看板才看得出主模型挂了；推理模型的思考链也计入消耗，否则成本看板少算钱。
- **流式读取**：结束原因 `finish_reason` 必须透传（正文被截断时界面全靠它提示）；流里混进解析不了的坏行要跳过，不能整条崩掉。
- **请求体**：只有显式要求 JSON 模式才带 `response_format`，`thinking` 字段同理——这两个字段部分第三方供应商不认，误发会直接报错。

这一批里最关键的一类断言是**「请求次数」**：重试多一次、降级多一轮，都直接对应真金白银。

### 修掉两个静默缺陷

**一、JSON 模式降级会把请求数翻倍（5 处真实链路受影响）**

代码注释写的是「若供应商不支持 `response_format`（通常 4xx），去掉 json 重试一次」，但实现对**任何**失败都重来一轮。

后果：遇到 429 限流或 5xx，`chat()` 内部已经重试 3 次，再补一轮就是 **6 次请求**。本来就被限流，加倍请求只会雪上加霜——越挤越挤。

现在只有「请求参数本身有问题」的 4xx 才降级（换掉参数确实可能就成了），429 / 5xx 不再叠加。受影响并因此受益的链路：选角、章纲生成、角色去重、编辑审核。

**二、退避等待超出标称值**

注释说「封顶 8 秒」，但抖动是加在封顶**之后**的，实际能到 9.6 秒。已改为抖动之后再封一次顶。

### 门禁

- `tsc` 0 错 · `vitest` **157 文件 1764 测试**全绿（新增 **70**）· `next build` 通过。
- 把 5 个原本私有的判定函数导出以便测试：`resolveMaxTokens` / `isRetryable` / `parseRetryAfter` / `backoffDelay` / `buildChain`。**只加 `export`，行为零变化。**

## v3.1.115 — 2026-09-11

### 首屏瘦掉九成：别为了一个版本号搬 800KB

**痛点**：`/changelog` 页面把 539 个历史版本一次性全渲染，HTML 有 **246 万字节**；更冤的是首页——它只想要一个 `LATEST_VERSION` 字符串，却被迫把 **819KB** 的历史版本数据一起打进首屏产物，因为两者写在同一个模块里，tree-shaking 摇不掉。为了拿一个版本号让首屏下 800KB，纯属白背包袱。

- 拆出 `src/lib/changelog-meta.ts`（几百行、几 KB）：只放最新版本号、摘要和类型定义。**全站唯一的版本号在这里定义**，`changelog-data.ts` 用 `export *` 转发出去——发版照旧只改一处，不存在「改了这里忘了那里」。
- 首页与 `/api/health` 改引这个轻量模块；只有 `/changelog` 页面才加载完整历史。
- `/changelog` 改**服务端分页**：默认首屏 20 条，「加载更早」每次加 40，「展开全部」才全量。页面由客户端组件改为服务端组件，按 `searchParams` 切片渲染。

**实测**：`/changelog` 首屏 **211,120 字节**（原 2,465,190，降 **91%**）；首页 JS 不再包含那个 819,110 字节的 changelog chunk，且对历史版本文案**零命中**。

### 补上第一层入站防护：137 个 API 从此只认本地与内网

**痛点**：项目共 137 个 API 路由，此前**没有任何一层入站鉴权**。在「数据本地零外泄、单机自托管」的定位下这原本可以接受——作者自己和自己之间不需要鉴权。但项目里带着 `deploy-local.ps1`，一旦被部署到局域网甚至公网，就变成任何人可读写全部稿件、还能白刷作者自己的 AI 额度。

- 新增 `src/proxy.ts`（Next 16 起 `middleware.ts` 已弃用，改名为 `proxy.ts`）+ `src/lib/net-guard.ts` 纯函数判定层。
- **放行**：`localhost`、`127.`、`10.`、`192.168.`、`172.16~31.`、`169.254.`（链路本地，直连/热点常见）、`::1`、`fe80:`、`fd`、`*.local`。手机连家里 WiFi 访问不受影响。
- **拦截**：其余来源一律 403，并回一条说清「为什么被拦 + 怎么解除」的中文提示，而不是只丢一个状态码让你猜。
- **只拦 `/api/*`**，页面与静态资源不受影响，本地使用零感知。
- **两条解除条件**：显式设 `ALLOW_PUBLIC=true`（自己清楚风险并主动承担，而不是被默认值坑）；或本来就跑在 Vercel 这类作者主动公开的托管平台——那里没有本地 SQLite 数据可泄，拦了只会把「只能看界面」的公开演示打成满屏 403，劝退想点 Star 的人。

### 两个不测就上线的坑

- **`next start` 会往 `x-forwarded-for` 里写 `::ffff:127.0.0.1`** 这种 IPv4 映射地址，而不是朴素的 `127.0.0.1`。少了归一化这一步，本机访问会被自己的防护拦成 403——防护没伤到外人，先把主人关在门外。已加 `normalizeAddress` 并钉死回归。
- 顺带把 Next 16 已弃用的 `middleware.ts` 迁到 `proxy.ts`，构建期弃用警告归零。

### 门禁

- `tsc` 0 错 · `vitest` **156 文件 1694 测试**全绿（新增 38）· `next build` 通过且零弃用警告。
- 新增 `src/lib/net-guard.test.ts` **39 条**：内网/公网名单逐项、IPv4 映射地址归一化（含「像映射但非法四段」不许还原，防止乱拼绕过）、xff 取第一个而非整条代理链、IPv6 的 Host 不被冒号切碎、空地址与脏数据一律当作不安全不做放行猜测、托管平台与显式开关的语义、403 文案必须说清怎么解除。
- 生产实测四项：本机访问 API **200** · 局域网来源 **200** · 伪装公网来源 **403** · 页面与静态资源照常 **200**；`/changelog` 首屏显示「已显示最近 20 / 539 个版本」，分页链接可用。

## v3.1.114 — 2026-09-11

### 去 AI 味检测：从「只告诉你哪儿有问题」到「一键帮你改掉」

**痛点**：规则引擎之前只会说「这里像 AI 写的，建议删掉」，真正的动手还得作者自己回编辑器选中、删除、再回来重跑。一万字稿子几十处痕迹 = 几十次来回，绝大多数人在搬第七趟时就放弃了——检测成了摆设。

- **新增执行层** `src/core/humanize/fixes.ts`（纯函数、零依赖）：`applyFixes` 把一批「机器有把握」的修复应用到原文。**多处同改时从后往前替换**——从前面改会让后面所有片段的下标偏掉，直接改到完全无关的原文上去；区间重叠的自动让位（只改一处），越界或空区间直接丢弃，宁可少改也不改坏。
- **22 个强特征词重新分档**，判据只有一条：**这个词在句子里是多余装饰，还是承重结构**。
  - 16 个属于前者（综上所述 / 不难发现 / 情不自禁 / 下意识…）——删掉句子照样站得住，给了 `machineFix`。**「宛如」不删，而是替换成「像」**。
  - 6 个属于后者（心头一紧 / 空气仿佛凝固 / 阳光透过树叶 / 斑驳的光影 / 心中一凛 / 仿佛整个世界）——它们本身就是句子的血肉，删掉只剩半截话，替换成什么又完全看作者想写什么。**这些依旧只给文字建议，界面不显示套用按钮**。
- **删除会顺手吞掉紧跟的逗号、顿号和「地」**。这条看着琐碎，实际是能不能用的分水岭：只删词的话，「值得注意的是，他来了」会变成「，他来了」，「她情不自禁地笑了」会变成「她地笑了」——改完比不改还难看。
- 另外两条规则也给了确定性改法：**破折号过多 → 换成逗号**（语义不断裂，最保守）；**括号补充过度 → 删掉整对括号**。

### 两个界面的不同处理

- **`/detector` 页面**：新增「一键套用 N 处」，改的是用户粘贴来的临时文本，**零风险、零网络**。套完立刻重算，分数肉眼可见地下降。每条命中各自也带一个小按钮，可以逐条决定要不要采纳。
  - 配了**撤销栈**：每次机器改动前把原文压栈，点「撤销」即回到上一版。敢让机器动稿子的前提，是随时能退回去。
- **工作台「本地过审自检」面板**：也支持套用，改动**直接写回本章正文**。这里刻意复用了手动编辑那条链路——同一个 PUT、同一把 `expectedVersion` 乐观锁、同一套 409 冲突面板，绝不因为「这是机器改的」就另开一条捷径，否则会悄悄覆盖作者刚在别处写入的内容。因为是批量改已落库的正文，这里保留了二次确认；改坏了还能走「版本历史」回滚。

### 门禁

- `tsc` 0 错 · `vitest` 155 文件 **1656** 测试全绿（新增 20）· `next build` 通过。
- 新增 `src/core/humanize/fixes.test.ts` **17 条**：吞尾巴符号、多处替换不错位、重叠让位、越界丢弃、**没带 fix 的命中一个字都不许动**、端到端套用后分数必须下降且不留病句、重复套用幂等（不会越改越短）。
- 新增 `HumanizePanel.test.tsx` **3 条**：没给写回通道时绝不画假按钮 / 套用后交给父组件的正文必须保留作者的句子主体 / 确认框上点取消则一个字都不改。

> 底线没动：本功能依然**纯本地、不联网、零额度**，`applyFixes` 只在作者点下按钮时才被调用，不存在「检测到什么就顺手改掉」这回事。机器拿不准的，永远留给作者。

## v3.1.113 — 2026-09-11

### 对比度收口（WCAG AA）+ 永久回归守卫

- **背景**：本站的 surface-* 令牌是半透明 rgba，而 axe 的 `color-contrast` 规则在 jsdom 里跑不了（无真实布局、拿不到叠加后的背景色），所以对比度长期只靠人工标注、**改坏了也没人知道**。本版改走「解析 CSS 变量 + 半透明合成 + 精算」，并把结论固化成自动化测试。
- **新增可复用模块** `src/lib/wcag-contrast.ts`（纯函数、零依赖）：解析六套主题的设计令牌，把半透明 `--nv-surface-*` 按 alpha 混合叠加到页面底色，再算 WCAG 相对亮度与对比度。
- **新增永久回归** `src/lib/wcag-contrast.test.ts`：**43 条断言覆盖 6 套主题**，固化四条契约—— ① `primary`/`secondary`/`tertiary` 落在**任何**表面都 ≥4.5:1；② `muted` 在页面底/容器面 ≥4.5:1；③ 备了 `muted-on-surface-3` 变体的主题，变体在浮起面必须达标；④ 没备变体的主题则必须自身全达标。另加「层级不倒挂」校验（primary > secondary > tertiary > muted）。
- **顺手抓出并修掉 3 处不达标**（都藏在四舍五入里、肉眼发现不了）：
  - `.reader-theme-sepia` 纸感：`muted #7d6c59` 三面**全部**不达标（void 4.40 / abyss 4.17 / surface-3 3.80），本主题无兜底变体、前几轮体检漏网 → 加深为 `#706150`（5.21 / 4.94 / 4.50）。
  - `.reader-theme-night` 夜间：`muted #8b8b98` 在浮起面 4.46（差 0.04）→ 提亮为 `#8c8c99`（4.52）。
  - `html.azure` 苍青：`muted #6A807C` 在容器面实测 **4.4967**；旧注释写的「4.8:1 达 AA」只对页面底成立，属四舍五入造成的假达标 → 提亮为 `#6a817c`（abyss 4.54），并订正了注释。
- **验证这组守卫不是摆设**：它一上线就抓出了苍青那个差 0.0033 的缺口。
- **门禁**：类型检查 0 错、vitest 154 文件 **1636** 测试全绿（新增 43）、next build 通过。
