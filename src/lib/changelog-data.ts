...
export const LATEST_VERSION = "v3.1.90";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "v3.1.90 应用修改升级为「按位置局部替换」+ 审稿支持指定单章（TARGETED-PATCH-AND-SINGLE-CHAPTER · 只改那一处，不再重写全文）：①应用修改不再整章重写——先让模型定位正文里要改的那一小段（给出与正文逐字一致的锚点），后端用子串精确匹配替换，其余一字不动；实测一条意见改下来 87.8% 原文原封不动（22 字那段换成 40 字，全文 181→199 字），整章重写做不到；②锚点没命中 / 定位失败会自动静默回退整章改写，不报错中断、不留半成品，篇幅异常膨胀或缩水同样判为锚点给错并触发回退；③结果说人话——不再是 UUID，改显示「✓ 第1章 柴房：局部替换（命中 1/1 处）」，一眼看出是精准替换还是整章改写、命中几条，照旧先存版本快照可回滚；④审稿范围新增「指定单章…」——点开是书里真实的章节列表（带字数），配套新增 GET /api/projects/[id]/chapters 轻量接口，只返回标题与字数、不带正文，不会因为选一章就把全书正文拉进浏览器；⑤门禁——类型检查 0 错、vitest 143 文件 1519 测试全绿（本轮新增 15 条：局部替换纯逻辑 9 / 章节清单 3 / 应用路由 3）、生产构建通过；端到端实测全过（临时项目建两章 → 指定单章审稿拿 3 条建议 → 局部替换 → 比对 87.8% 未动 → 快照可回滚 → 临时项目自动清理），全程没碰你的真实小说。个人 IP 仍归瑞宝宝。",
