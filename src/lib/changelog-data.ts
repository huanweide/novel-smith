    ...
    sections: [
      {
        label: "修复",
        items: [
          "右侧面板顶部 6 个 tab 在 320px 面板内被挤到每个 48px 宽，3 字标签（AI助手 / 工具箱）被迫换行成「AI助/手」「工具/箱」，中文词被拆断",
          "改为可换行布局：tab 容器加 flex-wrap，每个 tab flex-1 + min-w-[5.5rem] + whitespace-nowrap；6 个 tab 自动排成两行、标签完整显示；最小化按钮独立固定在右上角不受换行影响",
        ],
      },
      {
        label: "走查结论",
        items: [
          "页面层：12 个路由（首页/探讨/检测/拆书/拆书新建/工坊/回收站/设置/更新日志/写作台/阅读/数据表）全部 HTTP 200、零 JS 错误",
          "交互层：写作台 36 个可交互点（顶栏 7 + 左侧 10 + 大纲 2 + 工具栏 8 + 右侧 6 + 工具箱 3）逐一点击验证均有响应；分卷/平铺视图、微调模式、专注/沉浸模式、各对话框弹出均正常",
          "其余 8 个页面交互点统计正常（设置 13 按钮 / 检测 2 / 工坊 65 / 探讨 113 / 回收站 74 / 阅读 8），零 JS 错误",
          "排版层：全站无横向溢出；设置/工坊/检测/探讨零文字裁切；章节标题省略属设计内 truncate；未发现冗余或重复的交互控件",
        ],
      },
      {
        label: "门禁",
    ...
    sections: [
      {
        label: "修复",
        items: [
          "首页顶栏点「夜航」主题按钮后菜单不出现、无法切换主题。根因：装主题按钮的顶栏容器带 overflow-x-auto，该属性会把 overflow-y 一并计算为 auto，于是 absolute top-full 的下拉菜单被容器裁掉，点开也看不见",
          "主题菜单改用 React Portal 渲染到 document.body + fixed 定位（按按钮 getBoundingClientRect 计算），彻底脱离任何祖先 overflow / transform 裁剪；三处挂载点（首页顶栏 / 设置页外观区 / 系统状态横幅）一并受益",
        ],
      },
      {
        label: "交互增强",
        items: [
          "右键主题按钮快速循环切换下一档（夜航→白昼→苍青）；Esc 关闭菜单；滚动或缩放时菜单跟随按钮重新定位；菜单项新增 role=menuitemradio / aria-checked 语义与勾选态",
        ],
      },
      {
        label: "测试与门禁",
        items: [
          "新增 src/components/ui/ThemeToggle.test.tsx（5 例）：菜单必须挂在 document.body（防止再次落入 overflow 容器被裁）、选中后 html class 与 localStorage 生效、Esc 关闭、右键循环",
          "tsc 0 错 · vitest 175 文件 1853 测试全绿 · next build 通过",
        ],
      },
    ],
  },
