...
export const LATEST_VERSION = "v3.1.114";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "去 AI 味检测支持「一键套用」：把机器有把握的修改（删纯套话、破折号换逗号、删多余括号）一次点掉，不用再回编辑器手动选中删除",
  "新增 src/core/humanize/fixes.ts 执行层（纯函数零依赖）：从后往前替换保证多处同改时下标不错乱，区间重叠自动让位，越界修复直接丢弃",
  "22 个强特征词按「是冗余装饰还是承重结构」重新分档：16 个可机改，6 个（心头一紧/空气仿佛凝固/斑驳的光影等）删了句子会散架，仍只给文字建议",
  "删除会自动吞掉紧跟的逗号与「地」，避免改出「，他来了」「她地笑了」这类病句；20 条新回归把这条钉死",
];
