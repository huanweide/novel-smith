 ...
 * 「载入示例」用的样本文本。
 *
 * 两条硬要求（改动前请先想清楚，别顺手改短）：
 *  1. **字数必须明显超过 200 字**。破折号规则（rules.ts 的 detectDashOveruse）有
 *     `chars < 200 直接返回` 的生效门槛，样本一旦写短（旧版约 190 字）就一条破折号都不报，
 *     于是「一键套用」按钮根本不出现——新用户点完示例只能看到分数，
 *     看不到本页最核心的一步（SAMPLE-NO-FIXABLE，v3.1.124 黑箱实测暴露）。
 *  2. **必须同时命中「删除类」和「替换类」两种可机改项**：
 *     套话（值得注意的是 / 不由得 / 综上所述 → 删除）、破折号（—— → 换逗号）、
 *     替换词（宛如 → 像）。两种都演示到，用户才知道机器能改什么、不能改什么。
 */
const SAMPLE_TEXT = `夜幕降临，城市的霓虹灯渐次亮起。他站在窗前，静静地望着远方——那道光仿佛指引着他前行。值得注意的是，空气中的味道很复杂，有咖啡的香气，也有雨后的清新。不由得，他深深地吸了一口气，心中涌起一种难以言喻的情感。或许，这就是命运给予他的馈赠；又或许，这只是一场注定要醒来的梦——无论前路如何，他都将坚定地走下去。窗外的雨还在下，敲打着玻璃，一声接着一声。综上所述，唯有坚持才能抵达成功的彼岸，才能在漫长的岁月中找到属于自己的答案。他宛如一块沉默的礁石，任凭潮水来去。`;
