...
| 最新 Release | `v3.1.135 吸星改造 1：首页零门槛引导 + README 首屏重构（STAR-READINESS-1）`（2026-09-12，Latest）
...
### v3.1.135 — 2026-09-12 — 吸星改造 1：首页零门槛引导 + README 首屏重构（STAR-READINESS-1）
### v3.1.134 — 2026-09-12 — 全站 UI 走查收口：右侧面板 tab 断字修复（TAB-WRAP）
### v3.1.133 — 2026-09-12 — 对比模式：任意生成发现原有内容一律左右并排二选一（COMPARE-MODE）
### v3.1.132 — 2026-09-12 — 十档主题系统：夜航/苍青拉开差距 + 新增 7 套参考配色（THEME-10）
### v3.1.131 — 2026-09-12 — 修主题切换：点「夜航」菜单被顶栏裁掉、无法切换（THEME-MENU-CLIP）
### v3.1.130 — 2026-09-12 — 实时写作教练：把六维本地质量分析变成边写边看的体验（WRITING-COACH）
### v3.1.129 — 2026-09-12 — 表单标签横扫（五轮）：写作主面板 CenterPanel 3 个 placeholder-only 输入补 aria-label（FORM-LABEL-SWEEP-5）
### v3.1.128 — 2026-09-12 — 表单标签横扫（四轮）：导出/审稿面板 PublishCheckPanel 6 控件补齐 htmlFor / aria 关联（FORM-LABEL-SWEEP-4）
### v3.1.127 — 2026-09-12 — 表单标签横扫（三轮）：项目设定对话框 9 字段 + 标签搜索框补齐 htmlFor / aria-label 关联（FORM-LABEL-SWEEP-3）
### v3.1.126 — 2026-09-12 — 表单标签横扫（二轮）：三个面板补齐字段标签 / 可访问名 + FormLabel 提取为共享组件（FORM-LABEL-SWEEP / LABEL-WITHOUT-FOR）
### v3.1.125 — 2026-09-12 — 世界书新建表单补齐字段标签 + 提交按钮文案统一（FORM-NO-LABEL / BTN-TEXT-INCONSISTENT）
### v3.1.124 — 2026-09-12 — API 健壮性收口：读取路由补齐统一错误契约（ROBUST-UNGUARDED-ROUTES）+ Prisma P2025 记录不存在收敛为 404（P2025-SAYS-DB-DOWN）
### v3.1.123 — 2026-09-11 — 接口安全第四轮收口 + 首页 hydration 修复：generation-metrics 错误响应脱敏、ProjectCard 时间渲染确定性化（SEC-LEAK-GENMETRICS / FIX-HYDRATION-418）
### v3.1.122 — 2026-09-11 — 接口安全第三轮扫描收口：game/state 错误响应接入统一脱敏层（SEC-LEAK-GAMESTATE）
